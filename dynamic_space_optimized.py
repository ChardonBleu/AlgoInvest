...
class DynamicArray:
    ...
    def __init__(self, max_invest):
        ...
        """
        self.array = [
            [0 for column_invest in range(max_invest * 100 + 1)] for i in range(2)
        ]

    def build_dynamic_array(self, market, max_invest):
        """Fonction permettant d'obtenir le portefeuille d'actions le plus
        rentable en utilisant l'algorithme dynamique:
        On construit pas à pas un tableau des choix les plus rentables à partir
        de la solution la plus rentable déjà trouvée.
